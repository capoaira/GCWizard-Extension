...
window.addEventListener('contextmenu', function(e) {
    function handleResponse(message) {console.log('GCW response: '+message.response);}
    function handleError(error) {console.log('GCW Error: '+error);}
...
GCW.loadSettings = function () {
    function onError(e) {console.log('GCW Error: '+e);}
    function onGot(val) {
        // Default values
...
GCW.main = function() {
    ...
    // Add Sidebar
    function addSidebar() {
        let html = '<div class="CacheDetailNavigationWidget TopSpacing BottomSpacing">'
                 + '    <h3 class="WidgetHeader">'+GCW.i18n('extension_name')+'</h3>'
                 + '    <div class="WidgetBody">'
                 + '        <a href="javascript:void(0)" onclick="$(\'#gcw_analyze\').show(\'slow\');$(\'#aspnetForm\').hide(\'slow\');">'+GCW.i18n('analyze')+'</a>'
                 + '    </div>'
                 + '</div>';
    ...
    function addAnalyzePage() {
            ...
            if (isSource) {
                if ($('#ctl00_ContentBody_ShortDescription').html().trim() !== '') {
                    html += '<h3>'+GCW.i18n('analyze_short_description')+'</h3>'
                          + '<span>'+$('#ctl00_ContentBody_ShortDescription').html().toHtmlEntities()+'</span>'
                ...
                if (GCW.getVal('analyze_html_syntax')) {
                    ...
                    html = html.replace(/&#60;("[^"]*"|'[^']*'|[^'"&#62;])*&#62;/gm, function(match) {
                        match = match.replace(/(\w+)(=)(".*?")/gm, '<span class="gcw_html_attr">$1<span class="gcw_html_equal_sign">$2</span><span class="gcw_html_value">$3</span></span>');
        ...
        function changeComments(isOn) {
            if (isOn && !GCW.getVal('analyze_html_source')) { // Note dependency
                let html = $('#gcw_analyze_listing_content').html();
                html = html.replace(/<(!--.*--)>/gm,'$&<span class="gcw_html_comment">&lt;$1&gt;</span>');
                $('#gcw_analyze_listing_content').html(html);
                if (!html.match(/<!--.*-->/gm)) {
                    $('#analyze_show_comments').parents('.gcw_toggle label').after('<span class="gcw_toggel_warn">'+GCW.i18n('analyze_no_comments')+'</span>');
                }
            } else {
                let html = '';
                if ($('#ctl00_ContentBody_ShortDescription').html().trim() !== '') {
        ...
        if ($('#ctl00_ContentBody_lnkDH')[0].title == 'Decrypt') {
            ...
            hint = $('#div_hint').html();
            $('#ctl00_ContentBody_lnkDH')[0].click();
        } else hint = $('#div_hint').html();
        if (hint.trim() === '') hint = GCW.i18n('analyze_no_hint');
        // Add the html of the GCW analyze page
        let html = '<div id="gcw_analyze"><div style="margin:auto;width: max-content;">'
                 + '    <h1><a id="gcw_link_back" href="javascript:void(0)" onclick="$(\'#gcw_analyze\').hide(\'slow\');$(\'#aspnetForm\').show(\'slow\');">&lt;= '+GCW.i18n('analyze_back_to_listing')+'</a></h1>'
                 + '    <h1>'+GCW.i18n('analyze_owner')+'</h1>'
                 + '    <div id="gcw_analyze_owner">'
                 + '        <span>'+GCW.i18n('analyze_owner')+':</span><span>'+$('#ctl00_ContentBody_bottomSection a[href*="/play/search?owner[0]="]')[0].href.match(/\?owner\[0\]=(.*?)&/)[1].decodeUnicodeURIComponent()+'</span>'
                 + '        <span>'+GCW.i18n('analyze_pseudonym')+':</span><span>'+$('#ctl00_ContentBody_mcd1 a')[0].innerHTML+'</span>'
                 + '    </div>'
                 + '    <h1>'+GCW.i18n('analyze_description')+'</h1>'
                 ...
                 + '        <legend>'+GCW.i18n('analyze_description')+'</legend>'
                 + '        <div id="gcw_analyze_listing_content"></div>'
                 + '    </fieldset>'
                 + '    <div id="gcw_analyze_hint">'
                 + '        <div style="display:flex;align-items:center;gap:1em;"><h1>'+GCW.i18n('analyze_hint')+'</h1> '+GCW.i18n('analyze_hint_without_colors')+'</div>'
                 + '        <p>'+hint.replace(/<(?!(br|img)).*?>/gim, '')+'</p>'
                 + '    </div>'
                 + '</div></div>';
        $('body').append(html);
    ...
    $('body').append('<img id="gcw_visiter_counter" src="https://s04.flagcounter.com/count2/fPzU/bg_FFFFFF/txt_000000/border_CCCCCC/columns_5/maxflags_12/viewers_0/labels_0/pageviews_1/flags_0/percent_0/" style="display:none;"');
}
...
$(document).ready(function() {
    function waitForStorage(waitCount) {
        ...
        else {if (waitCount <= 100) {waitCount++;window.setTimeout(function(){waitForStorage(waitCount);}, 100);}}
    }
    waitForStorage(0);
});
